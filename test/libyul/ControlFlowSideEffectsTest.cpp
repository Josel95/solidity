...
TestCase::TestResult ControlFlowSideEffectsTest::run(ostream& _stream, string const& _linePrefix, bool _formatted)
{
	...
	for (auto&& [function, effects]: sideEffects.functionSideEffects())
	{
		string effectStr = toString(effects);
		result.emplace_back(
			function->name.str() + (effectStr.empty() ? ":" : ": " + effectStr) + "\n"
		);
	}
	sort(result.begin(), result.end());
	m_obtainedResult = util::joinHumanReadable(result, "");
